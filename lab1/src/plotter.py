...
def generate_plots(x_list, f_list, is_1d, f):
  ...
  if is_1d:
    ...
    for i in range(len(x)):
      for j in range(len(y)):
        Z[i,j] = f([x[i],y[j]])
    surf = ax.plot_surface(Y, X, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False, alpha=0.3)
